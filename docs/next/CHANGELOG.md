...
### Added
- Added `[ui.pane_border]` config with `mode` ("box" or "line"), `style` ("thick" or "plain"), `active_color`, and `inactive_color`. Line mode draws single-character split lines between panes with automatic junction characters; the focused pane's adjacent split-line segments use `active_color` while lines between unfocused panes use `inactive_color`.
