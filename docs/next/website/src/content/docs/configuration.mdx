...
Control how pane edges are drawn in multi-pane layouts.
...
|---|---|---|---|
| `mode` | `"box"`, `"line"` | `"box"` | `"box"` draws a full border block around each pane. `"line"` draws single-character split lines between panes (tmux-style); junction characters (`├┤┬┴┼` / `┣┫┳┻╋`) are chosen automatically based on neighbouring lines. |
| `style` | `"thick"`, `"plain"` | `"thick"` | Border character set. `"thick"` uses heavy box-drawing characters; `"plain"` uses thin ones. In `"box"` mode the focused pane uses the configured style while terminal input is active; unfocused panes and panes outside terminal mode use the plain style. In `"line"` mode the configured style applies to all split-line characters. |
| `active_color` | palette token (`accent`, `green`, `overlay0`, …) or literal color | `"accent"` | Color for split lines that border the focused pane. In `"box"` mode the focused pane's full border uses this color. In `"line"` mode only the split-line segments directly adjacent to the focused pane are highlighted; lines between two unfocused panes use `inactive_color`. |
| `inactive_color` | same as `active_color` | `"overlay0"` | Color for borders of unfocused panes. |
